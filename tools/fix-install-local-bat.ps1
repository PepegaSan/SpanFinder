# Regenerates install-local.bat as ASCII (fixes instant-close when file became UTF-16)
$bat = @"
@echo off
cd /d "%~dp0"
...
$out = Join-Path $root 'install-local.bat'
[System.IO.File]::WriteAllText($out, $bat.Replace("`n","`r`n"), [System.Text.Encoding]::ASCII)
Write-Host "OK: $out (ASCII)" -ForegroundColor Green